...
EMSCRIPTEN_KEEPALIVE
void wasm_add_book(const char *title, const char *author, int copies) {
    strncpy(books[book_count].title,  title,  TITLE_LEN - 1);
    strncpy(books[book_count].author, author, AUTHOR_LEN - 1);
    books[book_count].total_copies     = copies;
    books[book_count].available_copies = copies;
    books[book_count].book_id = (book_count == 0) ? 1001
                                : books[book_count - 1].book_id + 1;
    book_count++;
    save_books();
}

EMSCRIPTEN_KEEPALIVE
int wasm_issue_book(int book_id, const char *reg, const char *name) {
    int bi = -1;
    for (int i = 0; i < book_count; i++)
        if (books[i].book_id == book_id) { bi = i; break; }
    if (bi == -1) return -1;
    if (books[bi].available_copies <= 0) return -2;
    int len = (int)strlen(reg);
    if (len != 9) return -3;
    for (int i = 0; i < issued_count; i++)
        if (!issued[i].returned && issued[i].book_id == book_id &&
            strcmp(issued[i].student_reg, reg) == 0) return -4;
    ...
    rec.book_id  = book_id;
    strncpy(rec.book_title,   books[bi].title, TITLE_LEN  - 1);
    strncpy(rec.student_reg,  reg,             REG_LEN    - 1);
    strncpy(rec.student_name, name,            NAME_LEN   - 1);
    get_today(rec.issue_date);
    get_due_date(rec.due_date);
    rec.returned = 0;
    strcpy(rec.return_date, "N/A");
...
EMSCRIPTEN_KEEPALIVE
int wasm_return_book(int issue_id) {
    int ri = -1;
    for (int i = 0; i < issued_count; i++)
        if (issued[i].issue_id == issue_id) { ri = i; break; }
    if (ri == -1) return -1;
    if (issued[ri].returned) return -2;
    issued[ri].returned = 1;
    get_today(issued[ri].return_date);
    for (int i = 0; i < book_count; i++)
        if (books[i].book_id == issued[ri].book_id) {
...
EMSCRIPTEN_KEEPALIVE
const char* wasm_get_books_json() {
    static char buf[8192];
    strcpy(buf, "[");
    for (int i = 0; i < book_count; i++) {
        char entry[512];
        snprintf(entry, sizeof(entry),
            "%s{\"id\":%d,\"title\":\"%s\",\"author\":\"%s\",\"total\":%d,\"available\":%d}",
            i > 0 ? "," : "",
            books[i].book_id, books[i].title,
            books[i].author,  books[i].total_copies,
            books[i].available_copies);
        strcat(buf, entry);
    }
    strcat(buf, "]");
...
EMSCRIPTEN_KEEPALIVE
const char* wasm_get_issued_json() {
    static char buf[8192];
    strcpy(buf, "[");
    int first = 1;
    for (int i = 0; i < issued_count; i++) {
        if (issued[i].returned) continue;
        char entry[512];
        snprintf(entry, sizeof(entry),
            "%s{\"id\":%d,\"bookId\":%d,\"bookTitle\":\"%s\",\"reg\":\"%s\",\"name\":\"%s\",\"issueDate\":\"%s\",\"dueDate\":\"%s\"}",
            first ? "" : ",",
            issued[i].issue_id, issued[i].book_id,
            issued[i].book_title, issued[i].student_reg,
            issued[i].student_name, issued[i].issue_date,
            issued[i].due_date);
        strcat(buf, entry);
        first = 0;
    }
...
EMSCRIPTEN_KEEPALIVE
int wasm_get_book_count()   { return book_count; }

EMSCRIPTEN_KEEPALIVE
int wasm_get_issued_count() {
    int c = 0;
    for (int i = 0; i < issued_count; i++)
        if (!issued[i].returned) c++;
    return c;
}
